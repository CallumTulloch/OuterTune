...
「曲」画面では、**ライブラリ**、**DL済み**、**フォルダ**をそれぞれ独立して選択できる。
由来条件をすべてOFFにした状態は由来で絞り込まず、1つ以上ONにした場合はONにした条件同士を
ORで評価する。

**いいね済み** は由来とは別の条件として扱い、由来条件の結果とANDで組み合わせる。
**いいね済み OFF** は「いいねしていない曲」に絞る意味ではなく、いいね状態で絞り込まない。

| いいね済み | 由来条件 | 表示対象 |
| --- | --- | --- |
| OFF | すべてOFF | 全曲 |
| OFF | ライブラリON | ライブラリの全曲 |
| ON | すべてOFF | 全曲のうち、いいね済みの曲 |
| ON | ライブラリON | ライブラリの曲のうち、いいね済みの曲 |
